...
expr :: EITHER -> Tree Item -> Tree Item
expr lr a = do
  case a of
    (N Equ a b) -> do
      case lr of
        LEFT  -> (N Equ (expr lr a) b)
        RIGHT -> (N Equ a (expr lr b))
    (N Add c d) -> do
      let (come,r) = opeAdd c d
      case r of
        Just 1  -> (expr lr come)
        Nothing -> (N Add (expr lr come) (expr lr d))
    (N Sub e f) -> do
      let (come,r) = opeSub e f
      case r of
        Just 1  -> (expr lr come)
        Nothing -> (N Sub (expr lr come) (expr lr f))
    (L g)       ->  L g
...
data Item = Equ | Add | Sub | Mul | Div | Rpa | Lpa | Var String | Num Double deriving (Eq, Show)
